...
ollama_base_url = 'http://localhost:11434'              # since we are using litellm, we should not use v1 as if we do in OpenAI endpoint
ollama_api_key = os.getenv('OLLAMA_API_KEY')

MODEL = "ollama/llama3.2"           # litellm format of model name
db_name = "vector_db"
...
Provide detailed feedback and scores from 1 (very poor) to 5 (ideal) for each dimension. If the answer is wrong, then the accuracy score must be 1.""",
    ...
    judge_response = completion(model=MODEL, api_key=ollama_api_key, base_url=ollama_base_url, messages=judge_messages, response_format=AnswerEval)
